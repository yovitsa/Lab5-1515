"""
    ...
    Additionally, you must use try/except/else blocks to ensure that:
        ...
        1. Values entered by the user are numeric, and non-numeric (or blank) values do not crash the script
...
# height = height of the object, times = number of folds
# fold(0.1, 42 times)
...
def main():
    try:
        h = float(input("Please entern the height of the base:"))  
        n = int(input("Please enter the number of folds:"))
    except ValueError:
        print("Please enter numeric values:")
    else:
        resKm = fold(h,n) / 1000000
        tallerObject =  compare(resKm) # { "name" : "height"}
        if type (tallerObject) == dict:
...
if __name__ == '__main__':
   #termiante/end/kill the script you can type ctrl-c
   try:
      main()
